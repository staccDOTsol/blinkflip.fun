use anchor_lang::{prelude::*, solana_program::{program::invoke, program_memory::sol_memcmp, system_instruction, sysvar::SysvarId}};
...
fn parse_remaining_accounts<'info>(
    ...
    ref_count: u8,
    lut_count: u8,
) -> (&'info [AccountInfo<'info>], &'info [AccountInfo<'info>], &'info [AccountInfo<'info>]) {
...
fn update_lookup_tables<'info>(
    ...
    for account_info in luts.iter().rev() {
        ...
        if let Ok(_lookup_table) = AddressLookupTable::deserialize(&data) {
            if !lookup_table_table.lookup_tables.contains(&account_info.key()) {
                ...
                invoke(
                    &system_instruction::transfer(dev.key, lookup.key, lamports_diff),
                    &[
                ...
                lookup_table_table.lookup_tables.push(*account_info.key);
            }
        } else {
            break;
        }
